...
import com.pig4cloud.plugin.cache.util.CollectionUtil;
...
public class RedisCaffeineCache extends AbstractValueAdaptingCache implements Cache<Object, Object> {
    ...
    public Map<Object, Object> getAll(@NonNull Iterable<?> keys,
                                      @NonNull Function<? super Set<?>, @NonNull ? extends Map<?, ?>> mappingFunction) {
		GetAllContext context = new GetAllContext((Iterable<Object>) keys);
	...
	protected void doGetAll(GetAllContext context) {
		context.cachedKeyValues = caffeineCache.getAll(context.allKeys, keyIterable -> {
			Collection<Object> caffeineAbsentKeys = CollectionUtil.toCollection((Iterable<Object>) keyIterable);
			Collection<Object> redisKeys = CollectionUtil.mappedCollection(caffeineAbsentKeys, this::getKey);
			// 从 redis 批量获取
		...
		stringKeyRedisTemplate.executePipelined(new SessionCallback<Object>() {
			...
			public <K, V> Object execute(@NonNull RedisOperations<K, V> operations) throws DataAccessException {
				...
				map.forEach((k, v) -> {
					...
					setRedisValue(getKey(k), o, expire, valueOperations);
	...
	public void invalidateAll(@NonNull Iterable<@NonNull ?> keys) {
		Collection<?> keysColl = CollectionUtil.toCollection(keys);
		Collection<Object> redisKeys = CollectionUtil.mappedCollection(keysColl, this::getKey);
